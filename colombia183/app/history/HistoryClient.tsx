...
export default function HistoryClient({ initialTrips, userEmail }: { initialTrips: Trip[]; userEmail: string }) {
  ...
  const [filter, setFilter] = useState<"all" | "colombia" | "planned">("all");
  ...
  const totalPlanned = trips.filter(t => t.location === "planned").reduce((a, t) => a + getTripDays(t), 0);
    ...
    <>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "24px 16px 80px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: "#6b7280", marginTop: 2 }}>
              {trips.length} entries · {totalColombia} confirmed days · {totalPlanned} planned days
            </p>
          </div>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 20, background: "#16192a", padding: 4, borderRadius: 10, border: "1px solid #2a2d3e" }}>
          {([["all", "All"], ["colombia", "🇨🇴 Confirmed"], ["planned", "📅 Planned"]] as const).map(([v, l]) => (
            <button key={v} onClick={() => setFilter(v)} style={{
              flex: 1, padding: "8px", borderRadius: 8, border: "none", fontSize: 13, fontWeight: 600,
              background: filter === v ? "#FCD116" : "transparent",
              color: filter === v ? "#16192a" : "#6b7280",
              transition: "all .15s"
            }}>
              {l}
            </button>
